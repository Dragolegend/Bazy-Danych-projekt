...
Items_table.prototype.generate_table = function () {
    if (document.getElementById("items_table")) {
        document.body.removeChild(document.getElementById("items_table"));
    }

    this.table_object = document.createElement("table");
    this.table_object.id = "items_table";
    ...
    parent.app.items.forEach((e) => {
        ...
        const td_Faved = document.createElement("td");
        td_Faved.classList.add("field");
        td_Faved.classList.add("field_Faved");
        td_Faved.innerHTML = e.Faved;
        td_Faved.addEventListener("click", () => {
            if (parent.app.toggle_fav(e) && e.Faved) {
                td_Faved.innerHTML = 1;
            } else {
                td_Faved.innerHTML = 0;
            }
        });

        tr.appendChild(td_Faved);
    ...
    this.table_object = document.getElementById("items_table");
    
    this.filter_table();
}

Items_table.prototype.sort_table_letters = function (columnIndex) {
...
Items_table.prototype.sort_table_numbers = function (columnIndex) {
    const table = this.table_object;
    const rows = Array.from(table.rows).slice(1);
...
Items_table.prototype.parse_cell = function (value) {
    const parsedValue = parseFloat(value);
    return isNaN(parsedValue) ? value : parsedValue;
}

Items_table.prototype.filter_table = function () {
